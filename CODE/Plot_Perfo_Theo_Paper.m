function Plot_Perfo_Theo_paper(Def_Base,Results,Perf_Theo,Class,AlgName,Errorflag)
...
for ivar=1:length(Var_Name)
    ...
    h_plot(1) = plot(X(II),Y(II,3),'-r','linewidth',1.5); % la mediane

    %%  Ajustement du modele quadratique d'erreur
    P = polyfit(X(II),Var_Rmse_Est(II),2);
    ...
    Var_Name_Label = Var_Name{ivar};
        if ( strcmp(Var_Name{ivar},'LAI_Cab'))
                        Var_Name_Label = ['LAI*Cab'];
    elseif ( strcmp(Var_Name{ivar},'LAI_Cw'))
            Var_Name_Label = ['CWC'];
    end
    
    %title(['RMSE=(',num2str(P(1),'%0.4g'),').' Var_Name{ivar}  Errorflag '² + (',num2str(P(2),'%0.4g'),').' Var_Name{ivar}  Errorflag '+ (',num2str(P(3),'%0.4g'),')'],'FontSize',7)
    xlabel(['Estimated ' Var_Name_Label Errorflag] );
    ylabel([Var_Name_Label  Errorflag '_{Mes}-' Var_Name_Label  Errorflag '_{Est} & RMSE ']);
    plot([Min_xy Max_xy],[0 0],'-k')
