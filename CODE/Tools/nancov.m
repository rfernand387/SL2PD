...
%   any NaN values.  NANCOV(X,Y), where X and Y are vectors of equal
%   length, is equivalent to NANCOV([X(:) Y(:)]). 
%   
%   NANCOV(X) or NANCOV(X,Y) normalizes by (N-1) where N is the number of
%   observations after removing missing values.  This makes NANCOV(X) the
...
%   $Revision: 1.1.6.2 $  $Date: 2004/07/28 04:38:40 $
